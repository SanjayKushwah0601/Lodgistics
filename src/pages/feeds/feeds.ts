...
export class FeedsPage {
  ...
  updateHtmlTest(val, mentioned_targets) {
    ...
    let newValue = this.commonMethod.getTextValueWithNamesNew1(allChatMentions, this.members, val);
    if (newValue.text != "" && newValue.text.length > this.textLengthValue) {
      val = newValue.text.substring(0, this.textLengthValue);
      var newValueWrap = this.commonMethod.getTextValueNew1(allChatMentions, this.members, val);
      if (newValueWrap != "") {
        val = newValueWrap + "....";
      }
