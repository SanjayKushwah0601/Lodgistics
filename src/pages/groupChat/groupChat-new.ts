...
export class GroupChatPage {
  ...
  constructor(public platform: Platform, public navCtrl: NavController, private keyboard: Keyboard, public zone: NgZone, public navParams: NavParams, public nativeStorage: NativeStorage, private sqlite: SQLite, public alertCtrl: AlertController, public commonMethod: srviceMethodsCall, public translationservice: TranslationService, private viewCtrl: ViewController, private camera: Camera, private transfer: Transfer, private file: File, public actionSheetCtrl: ActionSheetController, private clipboard: Clipboard, public modalCtrl: ModalController, public events: Events, private iab: InAppBrowser) {
    ...
    if (this.navParams.get('groupInfo')) {
      ...
      this.sqlite.create({
        ...
        if (this.isPrivateChat == true) {
          // sqlQuery1 = "SELECT members.*,user_mentions.total FROM members LEFT JOIN user_mentions on user_mentions.user_id=members.user_id AND type='private' AND type_id='3' WHERE members.user_id IN('" + this.groupInfo.users[0].id + "','" + this.groupInfo.users[1].id + "') ORDER BY chat_group_users.created_at ASC";
  ...
  getMessagesFromLocalDb() {
    this.sqlite.create({
      ...
      // } else {
      //   selectChatQuery = "SELECT * FROM chat_messages WHERE target_id= '" + this.groupID + "' AND type='" + this.messageType + "' AND date(created_at)<= date('" + this.fromDate + "') AND date(created_at) >= date('" + this.fromDate + "') ORDER BY created_at DESC";
      // }
      if (this.callFirstTime) {
        selectChatQuery = "SELECT * FROM chat_messages WHERE target_id= '" + this.groupID + "' AND type='" + this.messageType + "' ORDER BY created_at DESC";
      } else {
...
console.log("userInfo lod log 1"+chatUserData);
  ...
  translate(sourceText, langCode, index) {
  ...
  sendImage(imageData) {
    ...
    if (window.screen.width < window.screen.height) {
      w = window.screen.width / 100 * 45;
    } else {
      w = window.screen.height / 100 * 45;
    }
    let thisObj = this;
    this.alert = this.alertCtrl.create({
      title: 'Message',
      message: '<img onmousedown="$event.preventDefault();"  width="' + w + 'px" height="' + w + 'px" src="' + imageData + '"/>',
...
console.log("userInfo lod log 2"+chatUserData);
  ...
  confirmWorkOrder(id, value, i, image_url, mentioned_user_ids) {
          ...
          buttons: [
            ...
            },
            {
              ...
              handler: data => {
                ...
                this.createWorkOrder(id, value, image_url, mentioned_user_ids, i);
              }
            }
          ]
        });
  ...
  createWorkOrder(id, value, image_url, mentioned_user_ids, i) {
    let modal = this.modalCtrl.create(CreateWorkOrderPage, { id: id, value: value, image_url: image_url, mentioned_user_ids: mentioned_user_ids, pageName: 'chat' });
    modal.onDidDismiss(data => {
      this.closekeyboard();
      this.isPopupOpen = false;
